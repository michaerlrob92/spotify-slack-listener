...
namespace SpotifySlackListener.Infrastructure
{
    ...
    {
        public static string GetEmojiForStatus(string status)
        {
            var words = status.Split(' ');
            foreach (var word in words)
            {
                var wordToMatch = word.ToLower().Trim();
                if (_emojiMap.ContainsKey(wordToMatch))
                {
                    return _emojiMap[wordToMatch];
                }
            }

            return _defaultEmojiMap.ElementAtOrDefault(status.Length % _defaultEmojiMap.Count());
        }

        private static IEnumerable<string> _defaultEmojiMap = new List<string>
        {
            ":headphones:",
            ":notes:",
            ":musical_note:"
        ...
        private static IDictionary<string, string> _emojiMap = new Dictionary<string, string>
        {
            {"alien", ":alien:"},
            {"ufo", ":alien:"},
